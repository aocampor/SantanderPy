...
import matplotlib, datetime, sys
...
def MakeRegressionsLoop(dfa, dfalabels, target):
    preds = {}
    accurs = []
    confMatr = []
    if printing:
    ...
    try:
        pre = Regression(dfa[1:limit_rows / 2],
                         dfalabels[target][1:limit_rows / 2],
                         dfa[limit_rows / 2:limit_rows], target
                             )
        acc = accuracy_score(dfalabels[target][limit_rows / 2:limit_rows], pre)
        matr = confusion_matrix(dfalabels[target][limit_rows / 2:limit_rows], pre)
        if plotting:
            PlotConfusion(matr, target)
        if printing:
            print "Predictions: "
            print pre
            print "Accuracy: "
            ...
            #pd.crosstab(pd.Series(dfalabels[target][limit_rows / 2:limit_rows], name='Actual'),
            #            pd.Series(pre, name='Predicted'),
            #            margins=True)
            print matr
            print(classification_report(dfalabels[target][limit_rows / 2:limit_rows], pre, target_names=['class0', 'class1']))
        #preds.append(pre)
        preds[target] = pre
        accurs.append(acc)
        confMatr.append(matr)
    except ValueError as e:
        print "There was an error with the regression " + target
        print e
        preds.append(0)
        accurs.append(-1)
        confMatr.append(0)
    if(plotting):            
        PlotList(accurs)
    SavePickle("parameters_"+ target + ".pickle", param)
...
def AccPlot(train, labels, n_estimators_range, param_name, label):
    ...
    if(nonzeros > 0):
        ...
        train_scores, test_scores = validation_curve(
            XGBClassifier(**param), train, labels, param_name=param_name,
    ...
    param[param_name] = n_estimators_range[entry]
    if printing:
...
if __name__ == "__main__":
    ...
    param = LoadPickle("parameters_"+target +".pickle")

    #PrintScatters(df)
    preds, accurs, confmatrix = MakeRegressionsLoop(df, dflabels, target)
